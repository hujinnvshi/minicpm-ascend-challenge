...
**2026-08-21 官方分支更新至 b06198f（必适配）**：新 RTF 口径（core 帧 pooled，基线样例 1.1~1.2）+ batch_validity 有效性契约（src_cnt 归帧/26 token/24000 samples/实时资格）+ 新提交规范（submission.zip 四件套 + §1.2 架构级复核通道）。我方 omni.cpp 缺归帧溯源字段（3-way merge 零冲突，产物在 /root/merge-prep，落地脚本 /root/repo-sync-pending/apply-sync.sh）。详见 docs/announcement-2026-08-21-official-update.md。
