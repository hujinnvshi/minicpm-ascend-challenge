...
赛题理解 → 资源准备 → 工具链掌握 → 本地优化 → 910C 验证 → 材料交付
   ✅ 已完     ✅ 已完     ✅ 已完       ✅ 收敛      ⏳ 排队中    8/15 起
...
- 算力：官方 HiDevLab 910C（100 卡时 = 1NPU 规格，已获批，排队中）
...
| 8/1-8/3 | 910C 审批等待期（备选：910B 预演启动判断） |
| 8/4-8/10 | 910C 验证 + 精度 + 正式数据（最优窗口） |
| 8/11-8/14 | 深度优化 + 报告定稿 |
| 8/15-8/17 | 材料收尾 + 提交（8/17 截止，每天限 3 次） |
