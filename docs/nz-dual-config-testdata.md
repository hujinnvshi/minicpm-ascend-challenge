...
| **TTS-Seed WER** | 全量2020 | **0.97%** ✅ | 1.501%(NZ=on 旧口径,作废) | ≤1.56 | **NZ=off 复跑定论：大幅优于基线（-31%）** |
| **TTS-Seed ASV** | 全量2020 | **0.708** ✅ | 0.694(NZ=on 旧口径) | ≥0.689 | **NZ=off 持平基线（-0.001）** |
...
- 现有数字（NZ=on 旧口径,作废）:WER **1.501%** / ASV **0.694**(全量 2020,官方 `run_tts_eval_cpp_zh.sh`)——该脚本 source `pipeline.env`,**无 NZ 设置 → NZ=on 生成**；**2026-08-15 NZ=off 全量复跑:WER 0.97% / ASV 0.708**（NZ=off 生成质量更好,官方 FAQ 正确性实锤）
