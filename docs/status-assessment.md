# 参赛状态评估（2026-07-31，08-26 最新：官方 910C 出分）

## 🏆 2026-08-26 官方成绩回写（终局评估，覆盖下文全部旧状态）

**官方 910C 实测出分（对应 8/22 提交，v5/v6 系配置）**：
...
**结论：三张入场券全部拿下——精度 4/4 全过准入 + 性能 beat 基线 23.1%，进入有效排名。**
- 本地对照：**同代码（v5 配置）910B4 1.3291 → 官方 910C 0.8357 = 1.59x（快 59%）**；
  （文档早先"1.166 vs 0.8357 快 28%"是 v6 本地 vs v5 官方的异代码对照，已修正）
- 精度全过的意义：官方脚本口径下 llama.cpp-omni 达到基线水平，之前 P8"准入标准不公"
  的担忧被官方数据推翻——**官方评测链路自洽**（本地自建脚本的 6.7%/崩溃是口径问题，非框架缺陷）
- 提交链：v3（探路）→ v4（FA）→ v5（8/22）→ v6（8/24 head 并行）→ v7（哈希修复，可选）

**剩余事项**：
1. v6 包 README 哈希无效（af67cfe 不存在，应为 1dd42b7）——若官方复核打回需 v7 重打包
2. 排行榜确认最终名次（2-3 天刷新，官方队列调度恢复中）
3. 提交额度：8/31 截止前每天限 3 次，若无更强配置不建议无谓重提交

---

# （历史记录，以下均为 2026-08-05 及更早状态，已被上方出分结论覆盖）

> ⚠️ **历史文档（2026-08-25 标记）**：本文件为 7/31-8/5 时代评估（910B3、P1.7 队列解耦、RTF 0.80），
> 已被 `competition-readiness.md` §0（2026-08-25 最新状态）取代。仅查早期决策背景用，勿引用其中数字。
