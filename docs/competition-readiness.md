# 赛事准备总览（2026-08-12 固化 · 2026-08-14 review-optimize 评审修订 · ⚠️ 2026-08-25 最新状态覆盖）

> ⚠️ **2026-08-25 更新**：下文"一、准入状态"及以后为 8/12-8/15 历史版（910B3、RTF 0.58-0.59、
> videomme 51.5% 均为旧口径/旧机器）。**当前真实状态见本文件顶部「§0 最新状态（2026-08-25）」**。
> 历史版保留备查，勿再引用其中的准入数字。

## §0 最新状态（2026-08-25，覆盖全文旧数字）

> 环境：**910B4 单卡（HBM 32GB，NPU id=3，NUMA node4→CPU 128-159）** + CANN 9.1.0-beta.3（官方要求 beta1，向上兼容）。
> 提交：v1→v6 共 6 版，**v5（8/22，RTF 1.3291）+ v6（8/24，RTF 1.166）在官方队列**（8/25 用户反馈从未被调度到评测，见 session-2026-08-25.md）。
> ⚠️ v6 包 README 最终提交哈希 af67cfe 无效（staging 已重建为 1dd42b7），v7 需修正。

### 准入状态（2026-08-25 实测口径）

| 项 | 官方基线 | 准入阈值 | 我们（最新） | 状态 |
|---|---|---|---|---|
| **Performance RTF**（排名核心） | 1.087 | <1.087 | **1.166**（v6 中位 4 run：1.139/1.151/1.181/1.186，官方 core 帧 pooled 口径） | ✅ beat 基线，连续 6 版递减 |
| **Daily-Omni** 精度 | 79.5 | ≥77.5 | **79.8%**（全量 1196 题） | ✅ |
| **TTS-Seed WER** | 1.414 | ≤1.56 | **0.97%**（全量 2020，NZ=off） | ✅ |
| **TTS-Seed ASV** | 0.709 | ≥0.689 | **0.708**（全量 2020，NZ=off） | ✅ |
| **Video-MME** | 69.0 | ≥67.0 | **72.9%**（官方 stratified 采样 ratio=0.1，197/270；旧 63.3% 为手动合池采样偏差） | ✅ |
| **Demo** | — | 端到端稳定 | 3 进程跑通 + 新机复验 | ✅ |

**结论：精度四项 + Demo 全达标；性能连续优化（1.71→1.166）；唯一风险 = 官方评测调度未出分 + v6 README 哈希。**

### 性能优化链（时间正序，全部 env 门控 + 双证据）

| 版本 | 优化 | 本机 core RTF | 证据 |
|---|---|---|---|
| 基线 | 官方默认 | 1.7143 | — |
| v4 (8/21) | OMNI_FORCE_FA=1（LLM Flash Attention） | 1.3785 | decode 0.571→0.234，精度零翻转 |
| v5 (8/22) | + OMNI_VISION_FA=1（VPM FA）+ OMNI_NPU_SERIAL=1（NPU 提交串行化） | 1.3291 | encode -8.2%，videomme 10/10 零翻转 |
| v6 (8/24) | + OMNI_HEADCODE_THREADS=24（TTS head_code 行间并行） | **1.166** | tts 0.417→0.275，logits 26/26 逐位一致，wav 20/20 逐字节一致 |

### 关键风险（2026-08-25）

1. 🔴 **官方调度未出分**：8/21 后榜单零新增；v5/v6 排队中，需重登查 state/日志
2. 🔴 **v6 README 哈希无效**：af67cfe 不存在，实际 staging HEAD=1dd42b7，复核打回风险
3. 🟡 截止 8/31（剩 6 天）；v5/v6 最快 8/26-8/27 出分
4. ⚪ VideoMME 达标依赖采样口径（官方正式 VIDEOMME_SAMPLE_RATIO 未公开）

---
# 历史版（2026-08-12/08-15，数字已被 §0 取代，备查）
...
| **Video-MME** 精度 | 69.0（全量2700）| ≥67.0 | **63.3%±5.7pp**（**270题合池**，NZ=off 官方路径；99q KB 域 51.5-53.5%）| ❌ 点估计未达（区间上沿 69 擦线）——基线口径申诉中（Q1-Q4，`organizer-inquiry-2026-08-15.md`）|
