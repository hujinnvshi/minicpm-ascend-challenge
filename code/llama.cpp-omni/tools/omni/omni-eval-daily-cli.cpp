// omni-eval-daily-cli.cpp
//
// A long-running, pipe-driven CLI for batch multimodal (video-frame + audio)
// evaluation of MiniCPM-o with llama.cpp-omni. Designed to replace the HTTP
// `llama-server` path used by the Daily-Omni evaluation pipeline.
//
// Relation to omni-eval-cli.cpp
// -----------------------------
// This is the Daily-Omni sibling of tools/omni/omni-eval-cli.cpp (the Video-MME
// eval CLI). Video-MME is vision-only; Daily-Omni interleaves one audio segment
// after every video frame. The two tools share the exact same protocol, channel
// separation and per-question reset logic; the only real difference is that this
// tool also prefills audio segments, interleaved with frames.
//
// Rationale
// ---------
// The evaluation harness prepares video frames AND per-frame audio segments on
// the Python side and then needs the model to: reset -> interleaved prefill of
// (frame, audio, frame, audio, ...) -> prefill the question text -> decode a
// short answer. Doing that over an HTTP/SSE server is wasteful (one network
// round-trip per media element, JSON (de)serialization, an HTTP stack, etc.).
//
...
//     {"type":"ping"}
//     {"type":"infer","id":"vid-1","frames":["/abs/f0.jpg", ...],
//      "audios":["/abs/a0.wav", ...],"prompt":"<question text>",
...
//
// Interleave semantics (mirrors the old HTTP prefill_interleaved, "v1")
// --------------------------------------------------------------------
// The Python evalkit builds content = [frame_0, audio_0, frame_1, audio_1, ...,
// text] and joins with "\n". We reproduce that by prefilling each media element
// followed by a standalone "\n" text prefill, i.e.
//     frame_0 "\n" audio_0 "\n" frame_1 "\n" audio_1 "\n" ... prompt
// which is exactly `"\n".join([frame_0, audio_0, ..., text])`. If there are more
// frames than audio segments, the trailing frames are prefilled the same way.
//
// NOTE (mel-normalization gap): each audio segment is encoded independently by
// stream_prefill (per-segment Whisper forward). The old HTTP "v2" path batched
// all segments through one 30s-chunk mel + Whisper forward to match Python's mel
// normalization / conv boundaries. That path required invasive server/omni.cpp
// changes and is intentionally NOT reproduced here — this CLI stays zero-intrusion
// on the inference framework, and the mel-normalization alignment is a known,
// deferred gap.
//
// Channel separation
// ------------------
...
// ---------------------------------------------------------------------------
// Model path resolution (same layout as omni-cli.cpp / omni-eval-cli.cpp)
...
// ---------------------------------------------------------------------------
// One question: reset -> interleaved (frame, audio) prefill -> prefill prompt
// -> decode. Returns the raw decoded text (answer parsing is on the Python side).
// ---------------------------------------------------------------------------
static bool run_infer(omni_context * ctx,
    ...
    // A standalone "\n" text prefill after each media element, so that the model
    // sees exactly `"\n".join([frame_0, audio_0, frame_1, audio_1, ..., text])`.
    auto prefill_newline = [&](void) -> bool {
        return stream_prefill(ctx, /*aud*/ "", /*img*/ "", idx++, /*max_slice*/ -1, /*text*/ "\n");
    };

    const size_t n_pairs = audios.empty() ? 0
                         : std::min(frames.size(), audios.size());
    ...
    for (size_t i = 0; i < frames.size(); i++) {
        ...
        if (!prefill_newline()) {
            out_err = "stream_prefill(newline after image) failed";
            return false;
        }

        // interleaved audio segment for this frame (v1: per-segment encoding)
        if (i < n_pairs) {
            const std::string & aud = audios[i];
            if (!file_exists(aud)) {
                out_err = "audio not found: " + aud;
                return false;
            }
            if (!stream_prefill(ctx, /*aud*/ aud, /*img*/ "", idx++)) {
                out_err = "stream_prefill(audio) failed: " + aud;
                return false;
            }
            if (!prefill_newline()) {
                out_err = "stream_prefill(newline after audio) failed";
                return false;
            }
        }
    }
...
static void show_usage(const char * prog) {
    fprintf(stderr,
        "MiniCPM-o Omni Daily-Omni Eval CLI - pipe-driven batch multimodal inference\n\n"
        ...
        "  --ref-audio <path>     Reference audio (system prompt voice slot; eval ignores TTS)\n"
...
int main(int argc, char ** argv) {
    ...
    fprintf(stderr, "=== omni-eval-daily-cli init ===\n");
    ...
    while (std::getline(std::cin, line)) {
        ...
        if (req.contains("audios") && req["audios"].is_array()) {
            for (const auto & a : req["audios"]) audios.push_back(a.get<std::string>());
        }
        ...
        bool ok = false;
        try {
            ok = run_infer(ctx, frames, audios, prompt, req_slice, req_npredict, text, err);
        } catch (const std::exception & e) {
            ok = false;
